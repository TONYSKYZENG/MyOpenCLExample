...
class MemAddresser {
 ...
 public:
  ...
  MemAddresser(uint8_t *_data, size_t lengthInBytes);
  ...
  void reset(void);
  void higherReset(uint8_t *data, size_t lengthInBytes);
  ...
  void writeAlignedBytes(uint8_t *input, size_t numberOfBytesToWrite);

  bool readAlignedBytes(uint8_t *output, size_t numberOfBytesToRead);
  ...
  bool readBits(uint8_t *output, size_t numberOfBitsToRead);
  template<typename T>
  T readValue(size_t numberOfBitsToRead) {
    T *ru;
  ...
  void writeBits(uint8_t *input, size_t numberOfBitsToWrite);
  template<typename T>
  void writeValue(T val, size_t numberOfBitsToWrite) {
  ...
  size_t numberOfBitsUsed;

  size_t numberOfBitsAllocated;
