...
MemAddresser::MemAddresser(uint8_t *_data, size_t lengthInBytes) {
  numberOfBitsUsed = 0;
  readOffset = 0;
...
void MemAddresser::writeBits(uint8_t *input, size_t numberOfBitsToWrite) {
  if (numberOfBitsToWrite <= 0)
    return;
  size_t offset = 0;
  uint8_t dataByte;
  size_t numberOfBitsUsedMod8;
...
bool MemAddresser::readBits(uint8_t *output, size_t numberOfBitsToRead) {
  uint8_t bitMask[] = {
  ...
  while (numberOfBitsToRead > 0) {
    ...
    *(output + offset) |= *(data + (readOffset >> 3)) >> (readOffsetMod8); // First half
    ...
    {
      size_t smallBits = 8 + numberOfBitsToRead;
...
void MemAddresser::higherReset(uint8_t *input, size_t lengthInBytes) {
  data = input;
  copyData = false;
  numberOfBitsUsed = 0;
  readOffset = 0;
...
void MemAddresser::write1(void) {
  size_t numberOfBitsMod8 = numberOfBitsUsed & 7;
...
void MemAddresser::writeAlignedBytes(uint8_t *input, size_t numberOfBytesToWrite) {
...
bool MemAddresser::readAlignedBytes(uint8_t *output, size_t numberOfBytesToRead) {
